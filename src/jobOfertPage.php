...
<body class="main-bg overflow-x-hidden">
    <main>
        ...
        <section class="w-full flex flex-col items-center mt-10 mb-20">
            <h1 class="font-Tiny5 uppercase text-5xl text-white mb-10">Wszystkie oferty pracy</h1>
            <div class="bg-gray-800 w-full rounded-xl px-8 py-12 max-w-[1000px] flex flex-col gap-5">
                <?php
            require "services/jobOfertService.php";
            $result = GetAllJobOferts();
            while($row = $result->fetch_assoc()){
                echo "<div class='bg-white rounded-xl px-8 py-8'>";
                echo "<h1 class='font-Tiny5 uppercase text-gray-700 text-4xl'>".$row['position_name']."</h1>";
                echo "<a href='jobOfertForm.php?job_ofert_id=".$row['job_ofert_id']."' class='bg-yellow-600 font-Tiny5 uppercase text-white text-xl px-8 py-2 mt-3 rounded-md flex max-w-[200px] justify-center'>Edytuj</a>";
                echo "</div>";
            }
            ?>
            </div>
